<template>
  <div class="row">
    <vue-table :title="$t('page.books')" :fields="fields" api-url="book" :item-actions="itemActions" @table-action="tableActions" show-paginate>
      <template slot="buttons">
        <router-link :to="{ name: 'dashboard.book.create' }" class="btn btn-sm btn-success" v-if="checkPermission('CREATE_BOOK')">{{ $t('page.create') }}</router-link>
      </template>
    </vue-table>
  </div>
</template>
